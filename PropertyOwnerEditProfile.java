/*
 * Class Name : PropertyOwnerEditProfile
 * Coder      : Jeniffer Teh Sue Ling
 * Purpose    : Allow property owner to edit profile
*/
...
public class PropertyOwnerEditProfile extends JFrame implements ActionListener, KeyListener {
    ...
    public PropertyOwnerEditProfile() {
        ...
        username = new JTextField(po.getUsername());
        ...
        fullName = new JTextField(po.getFullName());
        ...
        contactNum = new JTextField(po.getContactNum());
    ...
    public void actionPerformed(ActionEvent eve) {
        if (eve.getSource() == changePasswordButton) {
            new PropertyOwnerChangePassword();
            ...
            if (PotentialTenantRegister.checkJTextFieldFilled(username, userLbl) && PotentialTenantRegister.checkJTextFieldFilled(fullName, fullLbl)
                ...
                if (PotentialTenantRegister.checkUsernameExist(username.getText()) || po.getUsername().equals(username.getText())) {
                    JOptionPane.showMessageDialog(null, "Your new profile is saved.");
                    for (int i = 0; i < ownerList.size(); i++) {
                        if (po.getUsername().equals(ownerList.get(i).getUsername())) {
                            index = i;
                        }
                    }
                    ownerList.get(index).setUsername(username.getText());
                    ownerList.get(index).setFullName(fullName.getText());
                    ownerList.get(index).setContactNumber(contactNum.getText());
                    ownerList.get(index).setGender(PotentialTenantRegister.getUserGender(male));
    ...
    // Coder : Jenniffer Teh Sue Ling
    // Only allow property owner key in numbers at contact number textfield constructor and
    // Only allow property aowner key in alphabets at full name textfield constructor
