/*
 * Class Name : PropertyAgentEditProfile
 * Coder      : Chee Wei Jae
 * Purpose    : Allow Property Agent to edit profile
*/
...
public class PropertyAgentEditProfile extends JFrame implements ActionListener, KeyListener {
    ...
    public PropertyAgentEditProfile() {
        ...
        username = new JTextField(pa.getUsername());
        ...
        fullName = new JTextField(pa.getFullName());
        ...
        contactNum = new JTextField(pa.getContactNum());
    ...
    public void actionPerformed(ActionEvent eve) {
        if (eve.getSource() == changePasswordButton) {
            new PropertyAgentChangePassword();
            ...
            if (PotentialTenantRegister.checkJTextFieldFilled(username, userLbl) && PotentialTenantRegister.checkJTextFieldFilled(fullName, fullLbl)
                ...
                if (PotentialTenantRegister.checkUsernameExist(username.getText()) || pa.getUsername().equals(username.getText())) {
                    JOptionPane.showMessageDialog(null, "Your new profile is saved.");
                    for (int i = 0; i < agentList.size(); i++) {
                        if (pa.getUsername().equals(agentList.get(i).getUsername())) {
                            index = i;
                        }
                    }
                    agentList.get(index).setUsername(username.getText());
                    agentList.get(index).setFullName(fullName.getText());
                    agentList.get(index).setContactNumber(contactNum.getText());
                    agentList.get(index).setGender(PotentialTenantRegister.getUserGender(male));
