/*
 * Class Name : PropertyOwnerRegister
...
public class PropertyOwnerRegister extends JFrame implements ActionListener, KeyListener {
    ...
    static PropertyOwner owner = new PropertyOwner();
    private  UserControl uc = new UserControl();
    public PropertyOwnerRegister() {
        super("PROPERTY OWNER REGISTER");
    ...
    public static boolean checkUsernameExist(String u) {
        for (PropertyOwner t : owner.readOwnerList()) {
        ...
        if (eve.getSource() == registerButton) {
            ...
            && PotentialTenantRegister.checkJTextFieldFilled(contactNum, contactNumLbl) && PotentialTenantRegister.checkGenderSelected(male, female, genderLbl)) {
                if (checkUsernameExist(username.getText())) {
                    ...
                    PropertyOwner owner = new PropertyOwner(fullName.getText(), contactNum.getText(), PotentialTenantRegister.getUserGender(male),username.getText(), pass);
                   

                    try {
                        uc.pendingUserList(owner);
